...
export const createAppointmentInSupabase = async (appointment) => {
  ...
  // Clean payload matching Supabase PostgreSQL schema (UUID generated by DB)
  const dbPayload = {
    patient_name,
    patient_phone,
    patient_email,
  ...
  // 1. Direct Supabase insertion (guaranteed cross-device sync for mobile/desktop)
  try {
  ...
  // 2. API endpoint insertion for server synchronization
  try {
    ...
    const json = await res.json();
    if (json.data && !savedRecord) {
      savedRecord = json.data;
    }
  } catch (err) {}

  const resultData = {
    ...(savedRecord || { id: `app-${Date.now()}`, ...dbPayload, created_at: new Date().toISOString() }),
    patient_name,
...
export const createContactInquiryInSupabase = async (inquiry) => {
  ...
  try {
    ...
    const json = await res.json();
    if (json.data && !savedRecord) savedRecord = json.data;
  } catch (err) {}

  const resultData = {
    ...(savedRecord || { id: `inq-${Date.now()}`, ...dbPayload, created_at: new Date().toISOString() }),
    name,
    email,
    phone,
    subject,
